...
select avg(idade) from usuarios where idade >= 18;--usar o round tb
...
select categoria, sum(estoque) as "Soma Estoque" from farmacia where categoria = 'black' or categoria = 'blue' group by(categoria);
...
select count(medicamento) from farmacia where categoria is null;--'soma' text
...
select nome, idade, cadastro::date from usuarios where cadastro > '2020-01-01' and cadastro < '2021-01-01';
...
select nome, idade, email, age(cadastro::timestamp) from usuarios where idade >= 60;
...
select categoria, count(medicamento) from farmacia where categoria is not null group by(categoria);-- medicamento ou estoque
